...
const SYSTEM_PROMPT = `당신은 한국의 표준약관 및 공정거래 관련 법령(공정거래위원회 표준약관, 약관규제법, 저작권법, 하도급법, 상가건물임대차보호법 등)에 정통한 계약 검토 전문가입니다.
...
- basis: 법적 근거 또는 표준약관 근거 (예: "공정거래위원회 용역계약 표준약관 제○조", "저작권법 제45조")
...
export const analyzeContract = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const userPrompt = `계약 유형: ${data.contractType}\n\n--- 계약서 본문 ---\n${data.text}\n--- 끝 ---\n\n위 계약서를 분석하여 도구를 호출해 결과를 반환하세요.`;
